                    ...
                    <div class="flex justify-between items-start mb-8 pb-6 border-b border-gray-50">
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-2 text-sm">
                                ...
                                <p class="text-gray-500 font-bold"><span class="text-indigo-400">PERIOD:</span> {{ $project->tanggal_mulai }} <span class="text-pink-400">>></span> {{ $project->tanggal_selesai }} ({{ $project->durasi_hari }} Hari)</p>
                                <p class="text-gray-500 font-bold"><span class="text-indigo-400">MANAGER:</span> {{ $project->manager->name ?? 'N/A' }}</p>
                                <p class="text-gray-500 font-bold"><span class="text-indigo-400">ACUAN PEKERJAAN:</span> 
                                    @if($project->masterPekerjaan)
                                        <a href="{{ route('master-pekerjaans.show', $project->master_pekerjaan_id) }}" class="text-indigo-600 hover:underline">
                                            {{ $project->masterPekerjaan->nama_pekerjaan }} ({{ $project->masterPekerjaan->kode_pekerjaan }})
                                        </a>
                                    @else
                                        N/A
                                    @endif
